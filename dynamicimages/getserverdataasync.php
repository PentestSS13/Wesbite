...
foreach ($servers as $servername => $server) {
	...
	$socket_id = spl_object_id($socket);
	$socketserverarray[$socket_id] = $servername;
	$socketarray[$socket_id] = $socket;
...
foreach (array_diff(array_keys($socketarray), array_keys($w)) as $socket_id) 
		$serverdataarray[$socketserverarray[$socket_id]] = 'ERROR: connection error';
	...
	if($result != "") {
		if($result[0] == "\x00" || $result[1] == "\x83") { // make sure it's the right packet format
			...
			if($result[4] == "\x2a") { // 4-byte big-endian floating-point
				...
				$serverdataarray[$socketserverarray[$socket_id]] = 'ERROR: Unexpected int: '.$unpackint;
				continue;
			}
				...
				$serverdataarray[$socketserverarray[$socket_id]] = $unpackstr;
				continue;
			}
		}
	}	
	$serverdataarray[$socketserverarray[$socket_id]] = 'ERROR: read error';
}
...
foreach ($socketarray as $socket) {
    if (is_resource($socket) && get_resource_type($socket) === 'Socket') {
        ...
        echo "Socket has already been closed or is invalid.\n";
    }
}
