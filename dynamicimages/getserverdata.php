...
// Loop through the servers and get all the data we can.
// TODO: Use socket_select here for speed
...
foreach ($servers as $server) {
    $port = $server["port"];
    $addr = $server["address"];
	$lastinfo = (is_array($cache) && count($cache) > $n && array_key_exists($n, $cache)) ? $cache[$n] : [];
    $n++;
    ...
    foreach ($data_array as $item) {
        ...
        $row = explode("=", $item);
        $variable_value_array[$row[0]] = $row[1] ?? null;
    }
    
    $variable_value_array['cachetime'] = time();
    
    if (isset($variable_value_array['gamestate']) && $variable_value_array['gamestate'] == 4) {
